...
const loadOptionalDeployment = (name: string): _LiquityDeploymentJSON | undefined => {
  try {
    // Written by `hardhat deploy --network rhMainnet|rhTestnet`.
    return require(`../deployments/${name}.json`) as _LiquityDeploymentJSON;
  } catch {
    return undefined;
  }
};

const rhMainnet = loadOptionalDeployment("rhMainnet");
const rhTestnet = loadOptionalDeployment("rhTestnet");
...
const deployments: {
  ...
  ...(rhMainnet ? { [rhMainnet.chainId]: rhMainnet } : {}),
  ...(rhTestnet ? { [rhTestnet.chainId]: rhTestnet } : {})
};
