...
const versionFile = path.join(useLiveVersion ? "live" : artifacts, "version");
const contractsVersion = fs.existsSync(versionFile)
...
const deployerAccount = deployerKey
...
task("deploy", "Deploys the contracts to the network")
  ...
  .setAction(
    async ({ channel, gasPrice, useRealPriceFeed, createUniswapPair }: DeployParams, env) => {
      if (
        (env.network.name === "rhMainnet" || env.network.name === "rhTestnet") &&
        !process.env.DEPLOYER_PRIVATE_KEY &&
        !process.env.PRIVATE_KEY
      ) {
        throw new Error(
          "Set DEPLOYER_PRIVATE_KEY or PRIVATE_KEY in .env before deploying to Robinhood Chain"
        );
      }
      ...
      const deploymentPath = path.join("deployments", channel, `${env.network.name}.json`);
      fs.writeFileSync(deploymentPath, JSON.stringify(deployment, undefined, 2));
      fs.copyFileSync(deploymentPath, path.join("deployments", `${env.network.name}.json`));
